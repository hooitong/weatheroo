package co.realityshifters.weatheroo.utilities;
...
public class FakeDataUtils {
    ...
    private static ContentValues createTestWeatherContentValues(long date) {
        ...
        testWeatherValues.put(
                WeatherEntry.COLUMN_MIN_TEMP, maxTemp - (int) (Math.random() * 10));
        testWeatherValues.put(WeatherEntry.COLUMN_WIND_SPEED, Math.random() * 10);
        testWeatherValues.put(
                WeatherEntry.COLUMN_WEATHER_ID, weatherIDs[(int) (Math.random() * 10) % 5]);
    ...
    public static void insertFakeData(Context context) {
        ...
        for (int i = 0; i < 7; i++) {
            fakeValues.add(FakeDataUtils.createTestWeatherContentValues(
                    today + TimeUnit.DAYS.toMillis(i)));
        }
